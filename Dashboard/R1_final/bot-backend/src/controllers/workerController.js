...
exports.submitTaskWithMedia = async (req, res) => {
  try {
    const { taskId } = req.params;
    const { video_url, document_url, submission_notes } = req.body;
    if (!video_url) return res.status(400).json({ success: false, message: 'Video required' });
    const { supabase } = require('../config/supabaseClient');
    const { data, error } = await supabase.from('workers_tasks').update({
      video_url, document_url, submission_notes,
      submitted_at: new Date().toISOString(),
      status: 'submitted', admin_review_status: 'pending'
    }).eq('id', taskId).select().single();
    if (error) throw error;
    if (global.io) global.io.emit('new_submission', { taskId });
    res.json({ success: true, message: 'Submitted successfully', data });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.getTaskSubmission = async (req, res) => {
  try {
    const { supabase } = require('../config/supabaseClient');
    const { data, error } = await supabase.from('workers_tasks').select('*').eq('id', req.params.taskId).single();
    if (error) throw error;
...
exports.approveTaskSubmission = async (req, res) => {
  try {
    ...
    const { supabase } = require('../config/supabaseClient');
    const { data: task } = await supabase.from('workers_tasks').select('*, tickets(*)').eq('id', taskId).single();
    await supabase.from('workers_tasks').update({
      admin_review_status: 'approved', admin_review_notes,
      reviewed_by: 'admin', reviewed_at: new Date().toISOString(), status: 'completed'
    }).eq('id', taskId);
    if (task.ticket_id) {
      await supabase.from('tickets').update({
        status: 'resolved', resolution_video_url: task.video_url,
        resolution_document_url: task.document_url, resolved_at: new Date().toISOString()
      }).eq('id', task.ticket_id);
    }
    await supabase.from('worker_notifications').insert({
      worker_id: task.worker_id, task_id: taskId,
      title: ' Work Approved', message: 'Your work has been approved!', type: 'approval'
    });
    if (global.io) global.io.emit(`worker_notification_${task.worker_id}`, { type: 'approval', taskId });
    res.json({ success: true, message: 'Approved and sent to user' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.rejectTaskSubmission = async (req, res) => {
  try {
    ...
    const { admin_review_notes } = req.body;
    const { supabase } = require('../config/supabaseClient');
    const { data: task } = await supabase.from('workers_tasks').select('worker_id').eq('id', taskId).single();
    await supabase.from('workers_tasks').update({
      admin_review_status: 'rejected', admin_review_notes, reviewed_by: 'admin',
      reviewed_at: new Date().toISOString(), status: 'in_progress'
    }).eq('id', taskId);
    await supabase.from('worker_notifications').insert({
      worker_id: task.worker_id, task_id: taskId,
      title: ' Rejected', message: admin_review_notes, type: 'rejection'
    });
    if (global.io) global.io.emit(`worker_notification_${task.worker_id}`, { type: 'rejection', taskId });
...
exports.getPendingSubmissions = async (req, res) => {
  try {
    const { supabase } = require('../config/supabaseClient');
    const { data, error } = await supabase.from('workers_tasks')
      .select('*, workers(*), tickets(*)').eq('admin_review_status', 'pending')
...
exports.assignTicketToWorker = async (req, res) => {
  try {
    ...
    if (!ticket_id || !worker_id) return res.status(400).json({ success: false, message: 'Required fields missing' });
    const { supabase } = require('../config/supabaseClient');
    const { data: ticket } = await supabase.from('tickets').select('*').eq('id', ticket_id).single();
    const { data: task, error } = await supabase.from('workers_tasks').insert({
      worker_id, ticket_id, task_description: ticket.query,
      location: ticket.location, status: 'pending',
      assigned_at: new Date().toISOString(), deadline,
      priority: ticket.priority || 'medium'
    }).select().single();
    if (error) throw error;
    await supabase.from('tickets').update({ status: 'in-progress', assigned_worker_id: worker_id }).eq('id', ticket_id);
    await supabase.from('worker_notifications').insert({
      worker_id, task_id: task.id, title: ' New Task',
      message: `New: ${ticket.query?.substring(0, 50)}...`, type: 'assignment'
    });
    if (global.io) global.io.emit(`worker_notification_${worker_id}`, { type: 'new_task', taskId: task.id });
...
exports.updateWorkerLocation = async (req, res) => {
  try {
    ...
    if (global.io) {
      global.io.emit('workerLocationUpdate', {
        worker_id: workerId,
        latitude: lat,
        longitude: lng,
      ...
      console.log('📍 Broadcasted location update for worker:', workerId);
    }
